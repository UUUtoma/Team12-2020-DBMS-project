 ...
 */
PmEHash::PmEHash() {
	std::ifstream metadata_file(PM_EHASH_DIRECTORY META_NAME, std::ios::in);
	std::ifstream catalog_file(PM_EHASH_DIRECTORY CATALOG_NAME, std::ios::in);
...
void PmEHash::recover() {
	...
	catalog.buckets_pm_address = (pm_address*)pmem_map_file(path_ss.str().c_str(), sizeof(ehash_catalog), PMEM_FILE_CREATE, 0777, &map_len, &is_pmem);
 	// 读取所有数据页文件并内存映射
 	// 设置可扩展哈希的桶的虚拟地址指针
 	mapAllPage();
 	// 初始化所有其他可扩展哈希的内存数据
